...
import {Contact, NavigationProps} from "@/types/contacts";
...
export default function ContactsScreen() {
    ...
    const handleRedirect = (contact: Contact, route: "contact-show" | "contact-edit") => {
    ...
    return (
        <View className="p-3">
            <FlatList
                ...
                renderItem={({item}) => (
                    <View
                        className="flex flex-row justify-between items-center mb-4 p-4 rounded-lg bg-primary-100 dark:bg-primary-900 shadow-sm">
                        <View>
                            <Texts className="font-bold">{item.firstName} {item.lastName}</Texts>
                            <Texts className="pt-1">{item.email}</Texts>
                        </View>
                        <View className="flex-row gap-3">
                            <TouchableOpacity onPress={() => handleRedirect(item, "contact-show")}>
                                <Ionicons name="eye" size={24} color="#d0cdde"/>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => handleRedirect(item, "contact-edit")}>
